...
const memoMetadataFor = (memo, memoType) => (memo !== undefined && memo !== null && memo !== ''
...
const reservePayment = async ({
  ...
  try {
    ...
    await ledger.postPaymentReserved({ tx, transaction });
...
const executePayment = async ({
  ...
  const memoMetadata = memoMetadataFor(memo, memoType);

  const runCore = (tx) => reservePayment({
    tx,
    senderUser,
    normalizedAmount,
    effectiveAsset,
  ...
  const { quote, transaction } = await runDbTransaction(runCore);
  ...
  try {
    ...
    activeTransaction = await runDbTransaction(async (tx) => {
      ...
      await ledger.postPaymentSettled({ tx, transaction: updated });
      return updated;
    });
